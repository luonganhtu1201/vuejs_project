      ...
      <el-col :span="8">
        <el-card>
          <div class="update-avt">
            <Upload :imgUpdate="imgPreview" :reLoad="reLoad" @uploadImage="receiveImage"></Upload>
          </div>
          ...
          <el-table
            ...
            >
              <template slot-scope="scope">
                <img style="width: 45px;height: 45px" class="el-upload-list__item-thumbnail" :src="`http://vuecourse.zent.edu.vn/storage/${scope.row.image}`" alt="Ảnh">
              </template>
            </el-table-column>
            <el-table-column
                prop="name"
...
import Upload from "../lession02/Upload";
export default {
  name: "task01",
  components: {Upload},
  data () {
  ...
  methods: {
    ...
    editProduct(value){
      ...
      this.flag = false
      this.imgPreview = 'http://vuecourse.zent.edu.vn/storage/'+value.image
    },
    handleValidate(){
      let check = true;
    ...
    updateProduct(id) {
      if (this.handleValidate()){
        ...
        if (this.urlImage !== null){
          formData.append('image', this.urlImage);
        }
